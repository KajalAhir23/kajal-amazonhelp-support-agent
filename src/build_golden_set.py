...
Sampling method (documented for the report):
  ...
  2. Sample proportionally within a floor/ceiling per bucket (min 15,
     max 45) so no single intent dominates the golden set and no intent
...
def label_example(intent_bucket: str, conv: dict, interactive: bool) -> dict:
    if interactive:
        ...
        true_intent = input("True intent [enter to accept suggestion]: ").strip() or intent_bucket
        esc = input("Should escalate? (y/n): ").strip().lower() == "y"
